...
class PDFSimpliBot {
    ...
    // =============================================================
    // 🎯 DETECTOR MEJORADO DE MODALES DE ERROR
    // =============================================================
    async detectErrorModal() {
        await this.log("🔍 Buscando modal de error (detección mejorada)...");
        
        try {
            // 1. Buscar por texto en todo el documento
            const hasErrorText = await this.page.evaluate(() => {
                const errorKeywords = [
                    'declined', 'rejected', 'invalid', 'failed', 'error',
                    'unable', 'try again', 'card declined', 'payment failed'
                ];
            ...
            if (hasErrorText) {
                await this.log("🟡 Texto de error detectado en página");
                return true;
            }
            
            // 2. Buscar modales visibles
            const visibleModal = await this.page.evaluate(() => {
            ...
            if (visibleModal) {
                await this.log("🟡 Modal visible detectado");
                return true;
            }
            
            // 3. Buscar overlay oscuro (background de modal)
            ...
            if (hasOverlay) {
                await this.log("🟡 Overlay de modal detectado");
                return true;
            }
            ...
            await this.log(`⚠️ Error en detección de modal: ${error.message}`);
            return false;
        }
    }
}
...
// =============================================================
// VERIFICAR RESULTADO DE PAGO (ACTUALIZADO CON DETECCIÓN MEJORADA)
// =============================================================
PDFSimpliBot.prototype.verificarResultadoPago = async function (tarjetaActual) {
    ...
    // 🔥 USAR DETECCIÓN MEJORADA EN LUGAR DE closeErrorModal
    const hasErrorModal = await this.detectErrorModal();
    
    if (hasErrorModal) {
        // Intentar cerrar con múltiples métodos
        let modalClosed = false;
        
        // Método 1: Presionar Escape
        await this.log("🔼 Intentando cerrar modal con Escape...");
        await this.page.keyboard.press('Escape');
        await this.humanizer.delay(1000, 1500);
        
        // Método 2: Hacer clic fuera del modal
        await this.log("🖱️ Intentando clic fuera del modal...");
        await this.page.mouse.click(10, 10);
        await this.humanizer.delay(1000, 1500);
        
        // Método 3: Buscar botón con texto específico
        const closeButtons = [
        ...
        for (const selector of closeButtons) {
            try {
                await this.log(`🔍 Probando selector: ${selector}`);
                const element = await this.deepFind(selector);
                if (element) {
                    await element.click();
                    await this.humanizer.delay(1000, 1500);
                    modalClosed = true;
                    await this.log(`✅ Modal cerrado con selector: ${selector}`);
                    break;
                }
            } catch (e) {
                continue;
        ...
        if (modalClosed) {
            await this.log(`${COLORS_NEON.neonPink}❌ TARJETA INVÁLIDA — MODAL DETECTADO Y CERRADO (${cedulaTxt})${COLORS_NEON.reset}`);
            return false;
        }
    }
    ...
    await this.log(`${COLORS_NEON.neonPink}❌ TARJETA INVÁLIDA — TIMEOUT SIN ÉXITO (${cedulaTxt})${COLORS_NEON.reset}`);
    return false;
};
