...
class PDFSimpliBot {
    ...
    // =============================================================
    // FORMULARIO DE PAGO MEJORADO (MANEJO DE IFRAMES)
    // =============================================================
    async fillPaymentForm(cedula, mes, anio, ruc, nombre) {
        await this.log("🧾 Llenando formulario de pago...");
        await this.simulateHumanThinking();
        ...
        const intentar = async (intento) => {
            ...
            try {
                ...
                await this.log("📝 Escribiendo nombre del titular...");
                ...
                for (const selector of nameSelectors) {
                    const field = await this.deepFind(selector);
                    if (field) {
                        await field.click();
                        await this.humanizer.delay(200, 400);
                        await field.type(nombre, { delay: this.humanizer.random(60, 120) });
                        nameFieldFound = true;
                        break;
                    }
                }
                
                if (!nameFieldFound) throw new Error("Campo de nombre no encontrado");
                await this.humanizer.delay(800, 1200);

                // 2. Número de tarjeta (BUSCAR EN IFRAMES - STRIPE)
                await this.log("💳 Buscando campo de número de tarjeta...");
                
                // Primero, esperar a que carguen los iframes
                await this.delay(2000);
                ...
                for (const frame of frames) {
                    try {
                        // Intentar selectores comunes para campos de tarjeta
                        const cardSelectors = [
                        ...
                        for (const sel of cardSelectors) {
                            try {
                                ...
                                if (element) {
                                    await this.log(`✅ Encontrado campo tarjeta: ${sel}`);
                                    await element.click();
                                    await this.humanizer.delay(200, 400);
                                    // Limpiar campo si tiene algo
                                    await frame.evaluate(el => el.value = '', element);
                                    await this.humanizer.delay(100, 200);
                ...
                if (!cardFieldFound) {
                    // Último intento: buscar cualquier input que pueda ser para tarjeta
                ...
                await this.log("📅 Buscando campos de fecha de expiración...");
                ...
                const monthSelectors = [
                    ...
                    "[name='expiry-month']",
                    "select[data-field='expMonth']"
                ];
                
                let monthFound = false;
                for (const selector of monthSelectors) {
                    const monthSelect = await this.deepFind(selector);
                    if (monthSelect) {
                        await monthSelect.select(mes);
                ...
                if (!monthFound) {
                    // Intentar con input para mes
                    const monthInputs = [
                        "input[name='ccMonthExp']",
                        "input[placeholder*='MM']",
                        "input[data-field='expMonth']"
                    ];
                    
                    for (const selector of monthInputs) {
                        const monthInput = await this.deepFind(selector);
                        if (monthInput) {
                            await monthInput.click();
                            await this.humanizer.delay(200, 400);
                            await monthInput.type(mes, { delay: this.humanizer.random(60, 100) });
                            monthFound = true;
                            break;
                        }
                    }
                ...
                if (!monthFound) throw new Error("Campo de mes no encontrado");
                ...
                const yearSelectors = [
                    ...
                    "[name='expiry-year']",
                    "select[data-field='expYear']"
                ];
                
                let yearFound = false;
                for (const selector of yearSelectors) {
                    const yearSelect = await this.deepFind(selector);
                    if (yearSelect) {
                        await yearSelect.select(anio);
                        await this.humanizer.delay(300, 500);
                        yearFound = true;
                        break;
                    }
                ...
                if (!yearFound) {
                    // Intentar con input para año
                    const yearInputs = [
                        "input[name='ccYearExp']",
                        "input[placeholder*='YY']",
                        "input[placeholder*='YYYY']",
                        "input[data-field='expYear']"
                    ];
                    
                    for (const selector of yearInputs) {
                        const yearInput = await this.deepFind(selector);
                        if (yearInput) {
                            await yearInput.click();
                            await this.humanizer.delay(200, 400);
                            await yearInput.type(anio, { delay: this.humanizer.random(60, 100) });
                            yearFound = true;
                            break;
                        }
                    }
                ...
                if (!yearFound) throw new Error("Campo de año no encontrado");
                await this.humanizer.delay(400, 700);

                // 4. CVV (RUC en tu estructura)
                await this.log("🔑 Buscando campo CVV...");
                    ...
                    try {
                        ...
                        for (const sel of cvvSelectors) {
                            try {
                                const element = await frame.$(sel);
                                if (element) {
                                    await this.log(`✅ Encontrado campo CVV: ${sel}`);
                                    await element.click();
                                    await this.humanizer.delay(200, 400);
                ...
                if (!cvvFieldFound) {
                    // Último intento para CVV
                    const allInputs = await this.page.$$('input');
                    for (const input of allInputs) {
                        try {
                            const placeholder = await input.evaluate(el => el.getAttribute('placeholder') || '');
                            const maxlength = await input.evaluate(el => el.getAttribute('maxlength') || '');
                            
                            if ((placeholder.toLowerCase().includes('cvv') || 
                                 placeholder.toLowerCase().includes('cvc') ||
                                 placeholder.toLowerCase().includes('security')) &&
                ...
                await this.log("✓ Buscando checkbox de términos...");
                ...
                for (const selector of checkboxSelectors) {
                    const checkbox = await this.deepFind(selector);
                    if (checkbox) {
                ...
                if (!checkboxFound) await this.log("⚠️ Checkbox no encontrado, continuando...");
                await this.humanizer.delay(300, 600);

                // 6. Botón de submit/pago
                await this.log("🚀 Buscando botón de pago...");
                ...
                for (const selector of submitSelectors) {
                    const submitBtn = await this.deepFind(selector);
                    if (submitBtn) {
                        await this.log(`✅ Botón encontrado: ${selector}`);
                ...
                if (!submitFound) throw new Error("Botón de pago no encontrado");

                await this.humanizer.think(1500, 2500);
                await this.log("✅ Formulario de pago completado exitosamente");
...
PDFSimpliBot.prototype.fillCardFast = async function (cedula, ruc) {
    ...
    const intentar = async (intento) => {
        ...
        try {
            ...
            // Escribir CVV (RUC) (buscar en todos los frames)
            let cvvFieldFound = false;
            for (const frame of frames) {
                try {
                    const cvvSelectors = [
