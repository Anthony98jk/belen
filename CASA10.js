...
// ============================================================
// CONFIGURACIÓN VNC PARA CHROMIUM - RUTA CORREGIDA PARA TERMUX
// ============================================================
process.env.DISPLAY = ":1";

// Ruta real del Chromium instalado en Termux
const CHROMIUM_PATH = "/data/data/com.termux/files/usr/bin/chromium-browser";

// Comprobación de Chromium
if (!fs.existsSync(CHROMIUM_PATH)) {
    console.error("❌ ERROR: Chromium no existe en la ruta: " + CHROMIUM_PATH);
    console.error("👉 Ejecuta: pkg install chromium -y");
    process.exit(1);
} else {
    console.log("✅ Chromium encontrado en:", CHROMIUM_PATH);
}
...
class BotPDFSimpli {
    constructor() {
        ...
        // Iniciar servidor VNC si no está corriendo
        this.iniciarVNC();
    }
    ...
    async iniciarVNC() {
        return new Promise((resolve) => {
            ...
            exec("pgrep Xtigervnc || vncserver :1", (error, stdout, stderr) => {
                if (error) {
    ...
    // =======================================================================
    // 🔐 FUNCIÓN AUXILIAR: Delay aleatorio humano (antibot)
    // =======================================================================
    ...
    // =======================================================================
    // SISTEMA DE CUENTAS - VERSIÓN FINAL OPTIMIZADA
    // =======================================================================
    ...
    // =======================================================================
    // OBTENER PRÓXIMA CUENTA - VERSIÓN CORREGIDA (SIN MODIFICAR DURANTE FILTER)
    // =======================================================================

    obtenerProximaCuenta() {
        const ahora = Date.now();

        // PRIMERO: Hacer reset suave de cuentas con registro fallido después de 1 hora
        this.cuentas.forEach(c => {
        ...
        // LUEGO: Filtrar cuentas disponibles
        const cuentasDisponibles = this.cuentas.filter(c => {
            // 1. Cuentas validadas → nunca más
            if (c.validada) return false;

            // 2. Cuentas con registro fallido reciente → no usar
            if (c.registroFallido) return false;

            // 3. Cuentas bloqueadas por 3 fallos de tarjeta → verificar tiempo
    ...
    async iniciarNavegador() {
        return await this.ejecutarConReintentos(async () => {
            ...
            const options = {
                headless: false,
                executablePath: CHROMIUM_PATH, // ← RUTA CORREGIDA
    ...
    // =======================================================================
    // CLIC INTELIGENTE CON TIMEOUT - NUEVO MÉTODO AUXILIAR
    // =======================================================================
    ...
    async clicInteligenteConTimeout(selectores, descripcion, timeout = 5000) {
        ...
        while (Date.now() - startTime < timeout) {
            ...
            await this.delay(500); // Esperar medio segundo antes de reintentar
        }
    ...
    // =======================================================================
    // MANEJAR REGISTRO - VERSIÓN MEJORADA
    // =======================================================================
    ...
    async manejarRegistro() {
        return await this.ejecutarConReintentos(async () => {
            try {
                ...
                // Buscar campos de registro con timeout específico
                ...
                // Confirmar registro con timeout
                const registroConfirmado = await this.hacerClicRegistro(8000);
                ...
                // Validación robusta de éxito de registro
                const url = await this.page.url();
                const pageContent = await this.page.content();

                // Indicadores de registro exitoso
                ...
                if (registroExitoso) {
                    ...
                    // Reiniciar estado de la cuenta para procesar tarjetas
    ...
    // =======================================================================
    // HACER CLIC REGISTRO - VERSIÓN MEJORADA CON TIMEOUT
    // =======================================================================

    async hacerClicRegistro(timeout = 8000) {
        const selectores = [
    ...
    // ============================================================
    // CERRAR MODAL DE ERROR - FUNCIÓN DEFINITIVA PROFESIONAL
    // ============================================================

    async cerrarBotonCloseDespuesDeError() {
        this.log("🔍 Buscando botón 'Cerrar' en modal de error...");

        // 1. Múltiples selectores (texto Y clases similares)
        const selectores = [
        ...
        // ============================================================
        // LÓGICA PRINCIPAL: Probar múltiples selectores
        // ============================================================
        for (let i = 0; i < selectores.length; i++) {
            ...
            try {
                ...
                // 4) Intentar click mediante JavaScript
                try {
                ...
                // 5) Intentar clic forzado (dispatch)
                try {
                    await this.page.evaluate(el => {
                ...
                // 6) Verificar si REALMENTE se cerró
    ...
    // =======================================================================
    // PROCESAR RESULTADO DE TARJETA - SIN CAMBIAR LÓGICA, MÁS SEGURO
    // =======================================================================
    ...
    // =======================================================================
    // FLUJO PRINCIPAL - ESTABILIZADO Y SEGURO
    // =======================================================================
    async ejecutarProcesoCompleto() {
        try {
            ...
            while (tarjetasProcesadas < tarjetas.length) {
                ...
                const registroExitoso = await this.manejarRegistro();

                if (!registroExitoso) {
                    this.log(`❌ Falló registro: ${this.cuentaActual.email}`, 'ERROR');
                    this.guardarCuentas();
                ...
                this.log(`✅ Registro exitoso - Procesando tarjetas...`);
...
(async () => {
    console.log('========================================');
    console.log('🤖 BOT PDF SIMPLI - VERSIÓN VNC OPTIMIZADA');
    console.log('========================================');
    console.log('🖥️  Configurado para VNC (:1)');
    console.log('✅ Registro automático mejorado');
    console.log('🎯 Sistema de reCAPTCHA corregido');
    console.log('💳 Sistema de pago mejorado con PagoAuto');
    console.log('⚡ FLUJO RÁPIDO para tarjetas subsiguientes');
    console.log('🧹 LIMPIEZA INTELIGENTE de campos');
    console.log('🔍 VERIFICACIÓN PRO optimizada');
    console.log('📧 Email y contraseña humanos realistas');
    console.log('🔄 Sistema de cuentas mejorado');
    console.log('⚡ Optimizado para Termux + VNC');
    console.log('========================================');
