...
class BotPDFSimpli {
    ...
    // SISTEMA DE reCAPTCHA V2 CHECKBOX - VERSIÓN CORREGIDA (como TTTT.py)
    async manejarRecaptcha() {
        try {
            this.log('🔍 Buscando reCAPTCHA v2 checkbox...');
            await this.delay(2000);

            // ESTRATEGIA MEJORADA: Buscar todos los iframes como en TTTT.py
            ...
            for (const frame of frames) {
                try {
                    ...
                    if (frameUrl.includes('google.com/recaptcha')) {
                        ...
                        this.log(`📍 URL del iframe: ${frameUrl}`, 'DEBUG');
                        break;
                    }
                } catch (error) {
                    continue;
            ...
            // ESTRATEGIA SIMPLIFICADA COMO TTTT.py - solo hacer clic directo
            try {
                // Cambiar al iframe del reCAPTCHA
                await recaptchaFrame.waitForSelector('#recaptcha-anchor', { timeout: 5000 });
                
                // Hacer clic directo con JavaScript como en TTTT.py
                ...
                // Esperar breve tiempo como en TTTT.py
                await this.delay(3000);
                
                this.estadisticas.recaptchasResueltos++;
                this.log('✅ reCAPTCHA procesado - continuando flujo', 'SUCCESS');
                return true;
                
            } catch (error) {
                this.log('⚠️ No se pudo hacer clic automático en reCAPTCHA', 'WARNING');
                // CONTINUAR DE TODOS MODOS como en TTTT.py
                return true;
            }

        } catch (error) {
            this.logError(error, 'manejarRecaptcha');
            // EN CUALQUIER ERROR, CONTINUAR como en TTTT.py
            return true;
        }
    }
