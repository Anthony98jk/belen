...
class BotPDFSimpli {
    ...
    // SISTEMA DE REGISTRO - VERSIÓN ORIGINAL QUE FUNCIONABA
    async manejarRegistro() {
        return await this.ejecutarConReintentos(async () => {
            try {
                ...
                // MANEJAR reCAPTCHA CON EL MÉTODO QUE FUNCIONABA
    ...
    // SISTEMA DE reCAPTCHA V2 CHECKBOX - VERSIÓN QUE FUNCIONABA
    async manejarRecaptcha() {
        try {
            this.log("🔍 Buscando reCAPTCHA v2 checkbox...");
            await this.delay(1000);

            // Buscar el iframe anchor (nivel 1)
            ...
            // ESPERAR A QUE APAREZCA EL ANCHOR DENTRO DE ALGÚN SUB-IFRAME
            ...
            // Verificar si se abrió challenge
    ...
    // NUEVO MÉTODO: CONTINUAR A PÁGINA DE PAGO DESPUÉS DEL REGISTRO
    ...
    // FLUJO PRINCIPAL COMPLETO - CORREGIDO
    async ejecutarFlujoCompleto(tarjeta) {
        try {
            ...
            const pasos = [
                ...
                // NUEVO PASO: Solo si es necesario después del registro
                { nombre: "Continuar a página de pago", accion: () => this.verificarYContinuarAPago() },
    ...
    // NUEVO MÉTODO: Verificar si necesitamos continuar a la página de pago
    async verificarYContinuarAPago() {
        try {
            this.log('🔍 Verificando si estamos en página de pago...');
            await this.delay(3000);
            
            const url = await this.page.url();
            const contenido = await this.page.content();
            
            // Si ya estamos en una página de pago, no necesitamos hacer nada
            if (url.includes('payment') || url.includes('checkout') || url.includes('pago')) {
                this.log('✅ Ya estamos en página de pago', 'SUCCESS');
                return true;
            }
            
            // Verificar si hay elementos de pago visibles
            const elementosPago = await this.page.$$('input[placeholder*="card"], input[name*="card"], #planPageContinueButton');
            if (elementosPago.length > 0) {
                this.log('✅ Elementos de pago ya visibles', 'SUCCESS');
                return true;
            }
            
            // Si no estamos en página de pago, intentar hacer clic en continue
            this.log('🔄 No estamos en página de pago, intentando continuar...');
            return await this.hacerClicContinuePlan();
            
        } catch (error) {
            this.logError(error, 'verificarYContinuarAPago');
