...
// =============================================================
// CLOSE KILLER MEJORADO — DETECCIÓN INTELIGENTE DE MODALES
// =============================================================

PDFSimpliBot.prototype.findCloseButton = async function () {
    // 🔥 BUSCAR POR MÚLTIPLES PATRONES
    const cssSelectors = [
        // Selectores comunes de modales
    ...
    // Primero buscar en el frame principal
            ...
            for (const frame of frames) {
                try {
                    ...
                    for (const element of elements) {
                        ...
                        if (isVisible) {
                            await this.log(`✅ Botón Close encontrado: ${selector} en frame`);
                            return { element, selector, frame };
                        }
                    }
                } catch (e) {
    ...
    try {
        const closeElements = await this.page.evaluate(() => {
            ...
            for (const el of allElements) {
                ...
                // Elementos que probablemente son botones de cierre
                if ((text === 'close' || text === '×' || text === '✕' || text === 'x') && 
                    (tag === 'button' || tag === 'div' || tag === 'span' || tag === 'a')) {
        ...
        if (closeElements.length > 0) {
            ...
            const firstClose = closeElements[0];
            await this.log(`✅ Botón Close encontrado por texto: "${firstClose.text}" (${firstClose.tag}.${firstClose.className})`);
            
            // Encontrar el elemento en Puppeteer
            let selector = '';
            if (firstClose.id) {
                selector = `#${firstClose.id}`;
            } else if (firstClose.className) {
                const classStr = firstClose.className.split(' ').filter(c => c).join('.');
                if (classStr) selector = `.${classStr}`;
            }
            
            if (!selector) {
                // Selector por texto
                selector = `${firstClose.tag}:contains("${firstClose.text}")`;
            }
            
            const element = await this.deepFind(selector);
            if (element) {
                return { element, selector, frame: this.page };
            }
        }
    } catch (e) {
        await this.log(`⚠️ Error en búsqueda por texto: ${e.message}`);
    }

    // 🔥 BÚSQUEDA DE MODAL OVERLAY
    try {
        ...
        for (const selector of modalSelectors) {
            ...
            if (modal) {
                // Buscar botón de cierre dentro del modal
                const closeBtn = await modal.$('button, [class*="close"], [aria-label*="close"]');
                if (closeBtn) {
                    ...
                    if (isVisible) {
                        await this.log(`✅ Botón Close encontrado dentro de modal: ${selector}`);
                        return { element: closeBtn, selector: 'button dentro de modal', frame: this.page };
                    }
                }
            }
        }
...
PDFSimpliBot.prototype.closeErrorModal = async function () {
    await this.log("🔎 Buscando botón Close o modal de error...");

    // 🔥 PRIMERO: Tomar screenshot para debug
    try {
        const screenshotPath = path.join(this.baseDir, `debug_close_${Date.now()}.png`);
        await this.page.screenshot({ path: screenshotPath, fullPage: true });
        await this.log(`📸 Screenshot guardado: ${screenshotPath}`);
    } catch (e) {
        await this.log("⚠️ No se pudo tomar screenshot");
    }

    let foundAny = false;

    for (let intento = 0; intento < 8; intento++) {
        const result = await this.findCloseButton();
        ...
        foundAny = true;
        const { element, selector, frame } = result;
        
        await this.log(`🔴 Close detectado → click intento ${intento + 1} en: ${selector}`);

        try {
            // 🔥 INTENTAR MÚLTIPLES MÉTODOS DE CLIC
            await this.log(`🖱️ Intentando clic directo en elemento...`);
            
            // Método 1: Click directo con bounding box
            const box = await element.boundingBox();
            if (box) {
                await this.page.mouse.move(box.x + box.width / 2, box.y + box.height / 2);
                await this.humanizer.delay(100, 300);
                await this.page.mouse.down();
                await this.humanizer.delay(50, 150);
                await this.page.mouse.up();
                await this.log("✅ Clic realizado con coordenadas");
            } else {
                // Método 2: Click via evaluate
                await element.evaluate(el => el.click());
                await this.log("✅ Clic realizado via evaluate");
            }
            
            // Método 3: Click via frame si es diferente
            if (frame && frame !== this.page) {
            ...
            const stillVisible = await element.evaluate(el => {
                const style = window.getComputedStyle(el);
                return style.display !== 'none' && 
                       style.visibility !== 'hidden' &&
                       el.offsetWidth > 0 &&
            ...
            if (!stillVisible) {
                await this.log("✅ Modal cerrado exitosamente");
                return true;
            }
            
        } catch (clickError) {
            await this.log(`⚠️ Error en clic: ${clickError.message}`);
            
            // 🔥 FALLBACK: Cerrar modal por JavaScript
            try {
                await this.page.evaluate(() => {
                ...
                await this.log("✅ Modal eliminado via JavaScript");
                return true;
            } catch (jsError) {
                await this.log(`⚠️ Error eliminando modal: ${jsError.message}`);
            }
        }
    ...
    // 🔥 MÉTODO NUCLEAR: Remover todo lo que parezca modal
    await this.log("💣 MÉTODO NUCLEAR: Eliminando todos los elementos de modal...");
    ...
    try {
        await this.page.evaluate(() => {
            ...
            const selectorsToRemove = [
                ...
                '[role="dialog"]', '.paywall', '.popup',
                '.modal-backdrop', '.overlay', '.lightbox',
        ...
        await this.humanizer.delay(500);
        await this.log("✅ Todos los modales eliminados (Método Nuclear)");
        return true;
        
    } catch (e) {
        await this.log(`💥 Error en método nuclear: ${e.message}`);
        
        // Último recurso: recargar página
        await this.log("🔄 Recargando página como último recurso...");
