...
class PagoAuto {
    ...
    // ============================================================
    // FECHA DE EXPIRACION - VERSIÓN MEJORADA (FUNCIONAL PARA PDFSIMPLI)
    // ============================================================

    async buscarYCompletarFecha(tarjeta) {
        let completo = 0;

        const mesValue = tarjeta.mes.toString().padStart(2, "0");
        let anioValue = tarjeta.anio.length === 2 ? "20" + tarjeta.anio : tarjeta.anio;

        console.log(`🎯 Completando fecha - Mes: ${mesValue} | Año: ${anioValue}`);

        // SELECTORES EXACTOS DE PDFSIMPLI
        const selectorMes = "#expmo";
        const selectorAnio = "#expyr";

        // SELECCIONAR MES
        try {
            const selectMes = await this.page.$(selectorMes);
            if (selectMes) {
                console.log(`🔍 Dropdown MES detectado: ${selectorMes}`);
                await this.page.select(selectorMes, mesValue);
                console.log(`✅ Mes seleccionado: ${mesValue}`);
                completo++;
            } else {
                console.log("⚠️ No encontré selector de MES");
            }
        } catch (e) {
            console.log(`❌ Error seleccionando MES: ${e.message}`);
        }

        // SELECCIONAR AÑO
        try {
            const selectAnio = await this.page.$(selectorAnio);
            if (selectAnio) {
                console.log(`🔍 Dropdown AÑO detectado: ${selectorAnio}`);
                await this.page.select(selectorAnio, anioValue);
                console.log(`✅ Año seleccionado: ${anioValue}`);
                completo++;
            } else {
                console.log("⚠️ No encontré selector de AÑO");
            }
        } catch (e) {
            console.log(`❌ Error seleccionando AÑO: ${e.message}`);
        }

        // FALLBACK - INPUT COMBINADO
        if (completo < 2) {
            console.log("🔄 Intentando método alterno con input combinado...");
            ...
            for (const selector of posiblesSelectores) {
                try {
                    const inputFecha = await this.page.$(selector);
                    if (inputFecha) {
                        console.log(`🟩 Input combinado detectado: ${selector}`);
                        
                        const fecha = `${mesValue}/${tarjeta.anio.slice(-2)}`;
                        await inputFecha.click({ clickCount: 3 });
                        await inputFecha.type(fecha, { delay: 50 });
                        
                        console.log(`✅ Fecha combinada escrita: ${fecha}`);
                        completo = 2;
                        break;
                    }
                } catch (error) {
        ...
        console.log(`📊 Resultado fecha: ${completo}/2 campos completados`);
        return completo >= 2;
    }
    ...
    async completarInformacionPago(tarjeta) {
        return await this.ejecutarConReintentos(async () => {
            try {
                ...
                // 3. Fecha de expiración - NUEVA VERSIÓN MEJORADA
