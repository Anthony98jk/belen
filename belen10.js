...
class PagoAuto {
    ...
    async buscarYCompletarFecha(tarjeta) {
        ...
        for (const sel of selectsMes) {
            try {
                const select = await this.page.waitForSelector(sel, { timeout: 1000 });
                await select.select(tarjeta.mes.toString().padStart(2, '0'));
                completo++;
                break;
            } catch {}
        }
        ...
        for (const sel of selectsAnio) {
            try {
                const select = await this.page.waitForSelector(sel, { timeout: 1000 });
                let anio = tarjeta.anio;
                if (anio.length === 2) anio = "20" + anio;
                await select.select(anio);
                completo++;
                break;
            } catch {}
        }
