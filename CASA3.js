...
class BotPDFSimpli {
    ...
    // ============================================================
    // VERIFICACIÓN DE RESULTADO - VERSIÓN PRO OPTIMIZADA
    // ============================================================

    async verificarResultadoPago(tarjeta) {
        try {
            // 1) Esperar procesamiento inicial
            ...
            // ============================================================
            // 2) DETECCIÓN ABSOLUTA: URL de confirmación
            // ============================================================
            ...
            // ============================================================
            // 3) DETECCIÓN DEL PANEL DE "¡Error de Pago!"
            // ============================================================
            ...
            if (bloqueoPago) {
                this.log("❌🟥 Panel de Error de Pago detectado — TARJETA MALA");
                return false;
            }

            // ============================================================
            // 4) INDICADORES TEXTUALES DE ÉXITO
            // ============================================================
            ...
            for (const term of indicadoresExito) {
                if (lower.includes(term)) {
                    this.log(`💳💚 Éxito detectado: "${term}"`);
                    this.guardarTarjetaValida(tarjeta);
                    return true;
                }
            }
            ...
            // ============================================================
            // 5) DETECCIÓN DE BOTÓN DE DESCARGA (OPTIMIZADO)
            // ============================================================
            const botonesDescarga = await this.page.$x(
                "//button[contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'download')] | " +
                "//button[contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'descargar')] | " +
            ...
            if (botonesDescarga.length > 0) {
                this.log("💳💚 Botón de descarga encontrado — TARJETA VÁLIDA");
                this.guardarTarjetaValida(tarjeta);
                return true;
            }

            // ============================================================
            // 6) ICONOS DE CHECK / MENSAJES VISUALES DE ÉXITO
            // ============================================================
            const visualSuccess = await this.page.evaluate(() => {
                const t = (document.body.innerText || "").toLowerCase();

                // texto
                if (t.includes("thank you") || t.includes("gracias")) return true;
                if (t.includes("success") || t.includes("éxito")) return true;

                // íconos
                const icons = document.querySelectorAll("svg, img");
                for (const el of icons) {
                    const html = el.outerHTML.toLowerCase();
                    if (html.includes("check") || html.includes("success")) return true;
                }
            ...
            if (visualSuccess) {
                this.log("💳💚 Indicadores visuales de éxito detectados");
                this.guardarTarjetaValida(tarjeta);
                return true;
            }

            // ============================================================
            // 7) PATRONES DE URL DE ÉXITO (sin parámetros)
            // ============================================================
            const cleanUrl = currentUrl.split("?")[0];
            const patrones = ["/success", "/confirmation", "/complete", "/download"];

            if (patrones.some(p => cleanUrl.includes(p))) {
                this.log(`💳💚 URL patrón de éxito detectado`);
                this.guardarTarjetaValida(tarjeta);
                return true;
            }

            // ============================================================
            // 8) DETECCIÓN FINAL DE ERROR
            // ============================================================
            ...
            // ============================================================
            // 9) ULTIMA REGLA: si ya no estamos en checkout, es éxito
            // ============================================================
            if (!currentUrl.includes("/checkout") && !currentUrl.includes("/billing")) {
                this.log("💳💚 Redirección fuera de checkout → Pago probablemente exitoso");
                this.guardarTarjetaValida(tarjeta);
                return true;
            }

            // ============================================================
            // 10) Nada claro → tarjeta mala
            // ============================================================
