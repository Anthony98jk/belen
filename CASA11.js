...
class ChromiumVNCConfig {
    constructor() {
        this.CHROMIUM_PATH = "/data/data/com.termux/files/usr/bin/chromium-browser";
        this.DISPLAY = ":1";
        this.browser = null;
        this.page = null;
    }
    ...
    async capturarPantalla(nombreArchivo = 'screenshot.png') {
        await this.page.screenshot({ path: `/sdcard/Download/${nombreArchivo}`, fullPage: true });
    ...
    async diagnosticoCompleto() {
        ...
        try {
            await new Promise((resolve, reject) => {
                exec("pgrep Xtigervnc", (error, stdout) => {
                    if (error && !stdout) {
        ...
        try {
            fs.accessSync('/sdcard/Download', fs.constants.W_OK);
            console.log('✅ Permisos de escritura en /sdcard/Download OK');
        } catch {
            console.log('❌ Sin permisos en /sdcard/Download');
        }
...
// ============================================================
// CLASE PRINCIPAL DEL BOT - INTEGRADA CON VNC CONFIG
// ============================================================
class BotPDFSimpli {
    constructor() {
