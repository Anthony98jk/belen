...
class BotPDFSimpli {
    ...
    // Reglas reales:
    // 1) Cuentas que VALIDAN → NO SE USAN NUNCA MÁS
    // 2) Cuentas con 3 intentos fallidos → BLOQUEO 24 HORAS
    // 3) No se sobreescriben tarjetasProcesadas jamás
    // 4) ultimoUso solo se actualiza al terminar la cuenta
    // =======================================================================
    ...
    // =======================================================================
    // GENERACIÓN DE CORREOS ELECTRÓNICOS HUMANOS REALISTAS (Versión PRO)
    // =======================================================================
    generarListaCuentas() {
        ...
        for (let i = 0; i < 50; i++) {
            ...
            do {
                ...
                // Número de 3 a 4 dígitos para naturalidad humana
                num = Math.random() < 0.5
                    ? Math.floor(Math.random() * 900 + 100)        // 3 dígitos
                    : Math.floor(Math.random() * 9000 + 1000);     // 4 dígitos
    ...
    obtenerProximaCuenta() {
        ...
        const veinticuatroH = 24 * 60 * 60 * 1000;

        const cuentasDisponibles = this.cuentas.filter(c => {

            // Regla 1: si validó → jamás vuelve
            if (c.validada) return false;

            // Regla 2: si tiene 3 fallos → bloqueo 24h
            if (c.tarjetasProcesadas >= 3 && c.bloqueadaHasta) {
                if (new Date(c.bloqueadaHasta) > ahora) return false;
            }

            return true;
        });

        if (cuentasDisponibles.length === 0) return null;

        // Ordenar por:
        // 1) menos tarjetasProcesadas
        // 2) último uso más antiguo
        // 3) fecha de creación
    ...
    // =======================================================================
    // PROCESAR RESULTADO DE LA TARJETA
    // =======================================================================
    async procesarResultadoTarjeta(tarjeta, exito) {
        ...
        if (exito) {
            ...
            return; // NO sumar tarjetasProcesadas cuando valida
        }
        ...
        this.cuentaActual.tarjetasProcesadas++; // +1 fallo real

        // Bloqueo solo si NO está validada y llegó a 3 fallos
        if (!this.cuentaActual.validada && this.cuentaActual.tarjetasProcesadas >= 3) {
            const bloqueo = new Date(Date.now() + 24 * 60 * 60 * 1000);
            this.cuentaActual.bloqueadaHasta = bloqueo.toISOString();
            this.log(`⏰ Cuenta bloqueada por 24h: ${bloqueo.toLocaleString()}`, 'WARNING');
        }
    ...
    async ejecutarProcesoCompleto() {
        try {
            ...
            this.log('⚙ REGLAS: validada → nunca más | 3 fallos → 24h');
            ...
            while (tarjetasProcesadas < tarjetas.length) {
                ...
                this.log(`📊 Esta cuenta tiene ${this.cuentaActual.tarjetasProcesadas}/3 fallos`, 'INFO');
                ...
                // Actualizar último uso SOLO si no es validada
                if (!this.cuentaActual.validada) {
                    this.cuentaActual.ultimoUso = new Date().toISOString();
                }

                this.guardarCuentas();

                // Cerrar navegador para limpiar sesión
                if (this.browser) {
