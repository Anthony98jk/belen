...
class BotPDFSimpli {
    ...
    // FLUJO PRINCIPAL - SUBIR PDF (CORREGIDO PARA FRAME DETACHED)
    async subirPDF() {
        return await this.ejecutarConReintentos(async () => {
            try {
                ...
                // Buscar input file de forma más robusta
                let inputFile = await this.page.$('input[type="file"]');
                if (!inputFile) {
                    // Si no se encuentra, esperar y buscar de nuevo
                    await this.delay(2000);
                    inputFile = await this.page.$('input[type="file"]');
                }

                if (!inputFile) {
                    this.log('❌ No se encontró input file después de búsqueda extendida', 'ERROR');
                    return false;
                }

                // Verificar que el input esté disponible
                const isDisabled = await inputFile.evaluate(el => el.disabled);
                if (isDisabled) {
                    this.log('⚠️ Input file está deshabilitado, esperando...', 'WARNING');
                ...
                if (error.message.includes('detached')) {
                    ...
                    await this.delay(3000);
                    return false; // Para que se reintente
                }
    ...
    // NUEVO: CLIC EN CONTINUE DESPUÉS DE RECAPTCHA
    async hacerClicContinueDespuesRecaptcha() {
        const selectores = [
        ...
        return await this.clicInteligente(selectores, 'CONTINUE DESPUÉS RECAPTCHA', 3000);
    }

    // SISTEMA DE REGISTRO - CORREGIDO CON CONTINUE BUTTON
    async manejarRegistro() {
        return await this.ejecutarConReintentos(async () => {
            try {
                ...
                // Manejar reCAPTCHA de forma más agresiva
                const recaptchaResult = await this.manejarRecaptcha();
                if (!recaptchaResult) {
                    this.log('⚠️ Continuando a pesar del reCAPTCHA...', 'WARNING');
                }

                // CLIC EN CONTINUE BUTTON DESPUÉS DE RECAPTCHA
                this.log('🎯 Buscando botón CONTINUE después de reCAPTCHA...');
                await this.hacerClicContinueDespuesRecaptcha();
    ...
    // SISTEMA DE reCAPTCHA V2 CHECKBOX - MEJORADO
    async manejarRecaptcha() {
        try {
            this.log('🔍 Buscando reCAPTCHA v2 checkbox...');
            await this.delay(2000);

            // Buscar todos los iframes de reCAPTCHA
            ...
            for (const frame of frames) {
                try {
                    ...
                    if (frameUrl.includes('google.com/recaptcha') && (frameUrl.includes('api2/bframe') || frameUrl.includes('anchor'))) {
                        recaptchaFrame = frame;
                        this.log('✅ Iframe de reCAPTCHA encontrado', 'SUCCESS');
                        break;
                    }
                } catch (error) {
                    continue;
            ...
            if (!recaptchaFrame) {
                this.log('✅ No hay reCAPTCHA visible', 'SUCCESS');
                return true;
            }

            this.log('🎯 Intentando resolver reCAPTCHA automáticamente...');

            // Estrategia 1: Buscar el checkbox directamente en el iframe
            const checkbox = await recaptchaFrame.$('#recaptcha-anchor');
            if (checkbox) {
                this.log('✅ Checkbox de reCAPTCHA encontrado', 'SUCCESS');
                
                // Verificar si ya está marcado
                const estaMarcado = await recaptchaFrame.evaluate(() => {
                    const checkbox = document.querySelector('#recaptcha-anchor');
                    return checkbox ? checkbox.getAttribute('aria-checked') === 'true' : false;
                });
                ...
                // Hacer clic en el checkbox usando JavaScript
                ...
                this.log('✅ Clic en checkbox de reCAPTCHA realizado', 'SUCCESS');
                
                // Esperar a que se resuelva
                await this.delay(5000);
                
                // Verificar si se resolvió
                const resuelto = await recaptchaFrame.evaluate(() => {
                    const checkbox = document.querySelector('#recaptcha-anchor');
                    return checkbox ? checkbox.getAttribute('aria-checked') === 'true' : false;
                });
                
                if (resuelto) {
                    this.log('✅ reCAPTCHA resuelto automáticamente', 'SUCCESS');
                    this.estadisticas.recaptchasResueltos++;
                    return true;
                }
            }

            // Estrategia 2: Fallback - espera manual más corta
            this.log('⏳ reCAPTCHA no se pudo resolver automáticamente - esperando resolución manual (10 segundos)...');
            this.estadisticas.recaptchasManuales++;
            
            const startTime = Date.now();
            while (Date.now() - startTime < 10000) {
                // Verificar si el reCAPTCHA sigue visible
                const stillVisible = await this.page.$('iframe[src*="recaptcha"]');
                if (!stillVisible) {
                    this.log('✅ reCAPTCHA resuelto manualmente', 'SUCCESS');
                    return true;
                }
            ...
            this.log('❌ Tiempo agotado para reCAPTCHA', 'ERROR');
            return false;
            
        } catch (error) {
            this.logError(error, 'manejarRecaptcha');
    ...
    async marcarCheckboxSeguro() {
        try {
            ...
            return true; // Continuar aunque no se encuentre checkbox
    ...
    async ejecutarProcesoCompleto() {
        try {
            ...
            this.log('🤖 BOT PDF SIMPLI - VERSIÓN MEJORADA');
            this.log('✅ Registro automático forzado');
            this.log('🎯 Estrategia mixta para iframes');
            this.log('🛡️  Sistema automático de reCAPTCHA v2');
            this.log('⚡ Tiempos optimizados');
            this.log('🔒 Modo headless activado');
            this.log('🔄 CONTINUE button después de reCAPTCHA agregado');
...
(async () => {
    ...
    console.log('🛡️  Sistema automático de reCAPTCHA v2');
    console.log('📧 Email y contraseña aleatorios');
    console.log('🔒 Modo headless activado');
    console.log('🔄 CONTINUE button después de reCAPTCHA');
