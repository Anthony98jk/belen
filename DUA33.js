...
class Humanizer {
    ...
    async type(selector, text) {
        ...
        for (const char of text) {
            if (Math.random() < 0.04) {
                await this.page.type(selector, "x");
                await this.page.keyboard.press("Backspace");
                await this.delay();
            }

            await this.page.type(selector, char, { delay: this.random(60, 140) });
        }
...
class PDFSimpliBot {
    ...
    // =============================================================
    // FORMULARIO COMPLETO — CON HUMANIZER (CORREGIDO)
    // =============================================================
    async fillPaymentForm(cedula, mes, anio, ruc, nombre) {
        ...
        const intentar = async (intento) => {
            ...
            try {
                ...
                // 🔥 CORRECCIÓN: Buscar campo CVV (RUC) de forma más flexible
                ...
                // Si no se encuentra, buscar cualquier input con maxlength 3 o 4
                if (!cvvField) {
                    for (const f of frames) {
                        const inputs = await f.$$('input');
                        for (const input of inputs) {
                            const maxLen = await input.evaluate(el => el.getAttribute("maxlength") || "");
                            if (maxLen === "3" || maxLen === "4") {
                                // Verificar que no sea el campo de tarjeta
                ...
                if (!cvvField) throw new Error("Campo CVV no encontrado");

                // Escribir CVV (RUC) con humanizer
                await cvvField.click();
                await this.humanizer.delay(200, 400);
                await cvvField.type(ruc, { delay: this.humanizer.random(60, 120) });
        ...
        for (let i = 0; i < 3; i++) {
            ...
            try {
                await this.page.evaluate(() => {
                    ...
                    clean("input[maxlength='4']");
                    clean("input[maxlength='3']");
                });
            } catch (_) { }
...
// =============================================================
// FLUJO RÁPIDO — CON HUMANIZER (CORREGIDO)
// =============================================================
PDFSimpliBot.prototype.fillCardFast = async function (cedula, ruc) {
    ...
    const intentar = async (intento) => {
        ...
        try {
            ...
            // 🔥 CORRECCIÓN: Buscar campo CVV (RUC) de forma más flexible
            await this.log("🔍 Buscando campo CVV (modo rápido)...");
            
            // Intentar selectores específicos para CVV
            const cvvSelectors = [
                "input#data[name='Data']",
                "input[maxlength='4']",
                "input[maxlength='3']",
            ...
            if (!cvvField) throw new Error("Campo CVV no encontrado (rápido)");

            // Escribir CVV (RUC) directamente
            await cvvField.click();
            await this.humanizer.delay(200, 400);
            await cvvField.type(ruc, { delay: this.humanizer.random(60, 120) });
            await this.humanizer.delay(400, 700);
