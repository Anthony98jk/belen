...
class PagoAuto {
    ...
    // ============================================================
    // FECHA DE EXPIRACION - VERSIÓN PROFESIONAL MEJORADA
    // ============================================================
    ...
    async buscarYCompletarFecha(tarjeta) {
        ...
        // ------------------------------
        let mesValue = tarjeta.mes.toString().padStart(2, "0");    // "03" -> "03"
        // IMPORTANTE: Mantenemos el cero a la izquierda para los dropdowns
        let mesValueSinCero = parseInt(mesValue).toString();        // "03" -> "3" (backup)

        let anioValue = tarjeta.anio;
        if (anioValue.length === 2) {
            anioValue = "20" + anioValue;                          // "30" -> "2030"
        }

        this.log(`🎯 Fecha formateada -> Mes: ${mesValue} | Año: ${anioValue}`);

        let completo = 0;

        // ------------------------------
        // 2) Selectores confirmados PDFSimpli
        // ------------------------------
        const selectoresMes = [
        ...
        // ============================================================
        // 3) Completar MES - INTENTAR AMBOS FORMATOS
        // ============================================================
        for (const selector of selectoresMes) {
            try {
                ...
                if (exist) {
                    this.log(`🟦 MES encontrado en selector: ${selector}`);

                    // INTENTAR PRIMERO CON CERO (ej: "03")
                    try {
                        await this.page.select(selector, mesValue);
                        this.log(`✅ MES seleccionado con cero: ${mesValue}`);
                        completo++;
                        break;
                    } catch (error) {
                        // SI FALLA, INTENTAR SIN CERO (ej: "3")
                        try {
                            await this.page.select(selector, mesValueSinCero);
                            this.log(`✅ MES seleccionado sin cero: ${mesValueSinCero}`);
                            completo++;
                            break;
                        } catch (error2) {
                            this.log(`❌ No se pudo seleccionar mes en: ${selector}`);
                        }
                    }
                }
            } catch (_) {}
        ...
        for (const selector of selectoresAnio) {
            try {
                ...
                if (exist) {
                    this.log(`🟩 AÑO encontrado en selector: ${selector}`);

                    await this.page.select(selector, anioValue);
                    this.log(`✅ AÑO seleccionado correctamente: ${anioValue}`);
        ...
        // ============================================================
        // 5) Si ambos campos fueron completados → return true
        // ============================================================
        if (completo >= 2) {
            this.log("🔥 Fecha completada 100% correctamente.");
            return true;
        }

        // ============================================================
        // 6) FALLBACK: input combinado tipo MM/YY
        // ============================================================
        this.log("⚠️ No se encontró dropdown. Probando input combinado MM/YY…");
        ...
        const fechaCombinada = `${tarjeta.mes.toString().padStart(2, "0")}/${anioValue.slice(-2)}`;

        for (const selector of fallbackInputs) {
            try {
                await this.page.waitForSelector(selector, { timeout });
                ...
                if (input) {
                    ...
                    await input.type(fechaCombinada, { delay: 50 });

                    this.log(`🟨 Fecha combinada escrita: ${fechaCombinada}`);
        ...
        // ============================================================
        // 7) DEBUG AVANZADO - Mostrar qué elementos existen
        // ============================================================
        this.log("🔍 DEBUG: Analizando elementos de fecha en la página...");
        try {
            // Buscar todos los selects
            const selects = await this.page.$$('select');
            this.log(`📊 Encontrados ${selects.length} elementos <select>`);
            
            for (let i = 0; i < selects.length; i++) {
                const id = await selects[i].evaluate(el => el.id);
                const name = await selects[i].evaluate(el => el.name);
                this.log(`🎯 Select ${i}: id="${id}", name="${name}"`);
            }
            ...
            for (let i = 0; i < inputs.length; i++) {
                ...
                if (placeholder && (placeholder.includes('mm') || placeholder.includes('MM') || 
                    name && (name.includes('exp') || name.includes('month') || name.includes('year')))) {
                    inputsRelevantes.push(`Input ${i}: placeholder="${placeholder}", name="${name}"`);
                }
            }
            
            if (inputsRelevantes.length > 0) {
                this.log("🎯 Inputs relevantes encontrados:");
                inputsRelevantes.forEach(input => this.log(`   ${input}`));
            }
        } catch (e) {
            this.log("❌ Error en debug de elementos");
        }

        // ============================================================
        // 8) Si nada funcionó → return false
        // ============================================================
    ...
    async marcarCheckboxTerminos() {
        ...
        for (const sel of selects) {
            try {
                const campo = await this.page.waitForSelector(sel, { timeout: 2000 });
                const isChecked = await this.page.evaluate(el => el.checked, campo);
    ...
    log(mensaje) {
        console.log(`[PagoAuto] ${mensaje}`);
    }
}

// CLASE PRINCIPAL DEL BOT (se mantiene igual que antes)
class BotPDFSimpli {
    constructor() {
    ...
    // SISTEMA DE PAGO MEJORADO CON PAGOAUTO
    async completarInformacionPago(tarjeta) {
        return await this.ejecutarConReintentos(async () => {
            try {
                ...
                // 3. Fecha de expiración - NUEVA VERSIÓN PROFESIONAL
                ...
                if (await this.pagoAuto.marcarCheckboxTerminos()) {
                    camposCompletados++;
                    camposRequeridos.push('terminos');
                    this.log('✅ Términos aceptados', 'SUCCESS');
                } else {
                    this.log('⚠️ No se encontró checkbox de términos', 'WARNING');
                }
                ...
                if (exito) {
                    ...
                    this.log('❌ Información de pago incompleta', 'ERROR');
                }
    ...
    async hacerClicPago() {
        ...
        if (this.pagoAuto) {
            this.log('🔄 Usando PagoAuto para continuar...');
