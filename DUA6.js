...
class CaptchaSolver {
    ...
    async solve() {
        try {
            ...
            // PRIMERO: Intentar método específico de reCAPTCHA
                    ...
                    try {
                        for (const f of this.page.frames()) {
                            if (f.url().includes("recaptcha/api2/anchor")) {
                                ...
                                if (isChecked) {
                                    await this.log("🟢 CAPTCHA resuelto (método reCAPTCHA)");
                                    return true;
                                }
                            }
                        }
            ...
            await this.log("🔄 Usando método universal de CAPTCHA...");
...
// ======================================================
// 🔥 ADB MANAGER - CAMBIO DE IP CADA 6 TARJETAS
// ======================================================
class ADBManager {
    constructor(log = console.log) {
        this.log = log;
        this.tarjetaCounter = 0;
    ...
    async changeIP() {
        ...
        // Comandos ADB para cambiar IP (modo avión + datos)
        const commands = [
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
            ...
            // Esperar a que se establezca nueva conexión
            await this.delay(8000);
    ...
    async checkAndChangeIP() {
        ...
        // ✅ CAMBIO DE IP CADA 6 TARJETAS (ANTES DE LA 7ma)
        if (this.tarjetaCounter % 6 === 0) {
            await this.log(`${COLORS.yellow}📊 Tarjeta número ${this.tarjetaCounter} - SOLICITANDO CAMBIO DE IP${COLORS.reset}`);
            ...
            await this.log(`📊 Contador de tarjetas: ${this.tarjetaCounter}/6 para próximo cambio de IP`);
        }
...
class PDFSimpliBot {
    ...
    async initialize() {
        ...
        // Configurar user-agent para parecer más real
        ...
        await this.log("🚀 KATY20 — Iniciado en modo HEADLESS con sistema ADB integrado");
    }
    ...
    // =============================================================
    // VERIFICAR Y CAMBIAR IP CADA 6 TARJETAS
    // =============================================================
    async checkIPChangeRequired() {
        return await this.adbManager.checkAndChangeIP();
    }

    // =============================================================
    // SUBIR PDF (CON 5s EXTRA)
    // =============================================================
    async uploadPDF() {
    ...
    async verificarResultadoPago(tarjetaActual) {
        ...
        await this.delay(20000); // ✅ CAMBIADO DE 10 A 20 SEGUNDOS

        const startCheck = Date.now();
        const maxCheck = 20000; // ✅ AUMENTADO TAMBIÉN EL TIMEOUT MÁXIMO
        let success = false;
    ...
    async executeFlowCuenta() {
        ...
        while (this.cardsThisAccount < 3) {
            // ✅ VERIFICAR CAMBIO DE IP CADA 6 TARJETAS (ANTES DE PROCESAR TARJETA)
            const ipOk = await this.checkIPChangeRequired();
            if (!ipOk) {
