...
// ============================================================
// CONFIGURACIÓN CHROMIUM PARA PC - SIN VNC
// ============================================================
class ChromiumPCConfig {
    constructor() {
        // RUTAS PARA PC CON CHROMIUM
        this.CHROMIUM_PATH = "/snap/bin/chromium";
        this.browser = null;
        this.page = null;
    }
    ...
    verificarChromium() {
        ...
        if (!fs.existsSync(this.CHROMIUM_PATH)) {
            ...
            console.error("👉 Ejecuta: sudo snap install chromium");
            return false;
        } else {
    ...
    getOpcionesNavegador() {
        const options = {
            headless: false, // Cambiar a true si quieres modo headless
            executablePath: this.CHROMIUM_PATH,
            ignoreHTTPSErrors: true,
            defaultViewport: null,
            userDataDir: '/home/anthony/chromium-profile',
            args: [
                "--start-maximized",
                ...
                "--user-data-dir=/home/anthony/chromium-profile"
            ]
        };
    ...
    async iniciarNavegador() {
        try {
            ...
            console.log('✅ Navegador configurado correctamente', 'SUCCESS');
    ...
    async capturarPantalla(nombreArchivo = 'screenshot.png') {
        const rutaCapturas = path.join('/home/anthony/botdata', nombreArchivo);
        await this.page.screenshot({ path: rutaCapturas, fullPage: true });
        console.log(`📸 Captura guardada: ${rutaCapturas}`);
    }
    ...
    async diagnosticoCompleto() {
        ...
        try {
            const rutaBotdata = '/home/anthony/botdata';
            fs.accessSync(rutaBotdata, fs.constants.W_OK);
            console.log('✅ Permisos de escritura en botdata OK');
        } catch {
            console.log('❌ Sin permisos en botdata');
        }
...
// ============================================================
// CLASE PAGOAUTO MEJORADA (SIN CAMBIOS EN LÓGICA)
// ============================================================
class PagoAuto {
    constructor(page) {
        this.page = page;
...
// ============================================================
// CLASE PRINCIPAL DEL BOT - SIN VNC
// ============================================================
class BotPDFSimpli {
    constructor() {
        this.pcConfig = new ChromiumPCConfig();
        
        // RUTAS PARA PC
        this.rutaPdf = '/home/anthony/botdata';
        this.archivoTarjetas = '/home/anthony/botdata/tarjetas.txt';
        this.archivoLives = '/home/anthony/botdata/lives.txt';
        this.archivoCuentas = '/home/anthony/botdata/cuentas_pdfsimpli.json';
        this.archivoProxies = '/home/anthony/botdata/proxies.txt';
        this.archivoLog = '/home/anthony/botdata/bot_log.txt';
    ...
    crearArchivosNecesarios() {
        try {
            // Crear directorio botdata si no existe
            const botdataDir = '/home/anthony/botdata';
            if (!fs.existsSync(botdataDir)) {
                fs.mkdirSync(botdataDir, { recursive: true });
                this.log('📁 Directorio botdata creado');
            }
            ...
            if (!fs.existsSync(this.archivoTarjetas)) {
                ...
                this.log('📁 Archivo de tarjetas creado en botdata');
            }
            ...
            if (pdfs.length === 0) {
                this.log('⚠️ No hay PDFs en la carpeta botdata. Coloca al menos un PDF allí.', 'WARNING');
            } else {
                this.log(`✅ ${pdfs.length} PDF(s) encontrado(s) en botdata`);
            }

        } catch (error) {
            this.log('❌ Error creando archivos necesarios: ' + error.message, 'ERROR');
        }
    }
    ...
    cargarOGenerarCuentas() {
        try {
            ...
            this.log('❌ Error cargando cuentas, generando nuevas...');
            this.generarListaCuentas();
            this.guardarCuentas();
        }
    }
    ...
    guardarCuentas() {
        try {
            ...
            this.log('💾 Cuentas guardadas correctamente');
        } catch (error) {
            this.log('❌ Error guardando cuentas: ' + error.message, 'ERROR');
        }
    }
    ...
    async iniciarNavegador() {
        return await this.ejecutarConReintentos(async () => {
            ...
            const { browser, page } = await this.pcConfig.iniciarNavegador();
            this.browser = browser;
            this.page = page;

            this.pagoAuto = new PagoAuto(this.page);

            this.log('✅ Navegador configurado correctamente', 'SUCCESS');
            return true;
    ...
    async subirPDF() {
        return await this.ejecutarConReintentos(async () => {
            try {
                ...
                if (pdfs.length === 0) {
                    this.log('❌ No hay PDFs en la carpeta botdata', 'ERROR');
                    return false;
                }
    ...
    async ejecutarProcesoCompleto() {
        try {
            ...
            if (tarjetas.length === 0) {
                this.log('❌ No hay tarjetas para procesar', 'ERROR');
                return;
            }
...
// ============================================================
// EJECUCIÓN PRINCIPAL - SIN VNC
// ============================================================
(async () => {
    console.log('========================================');
    console.log('🤖 BOT PDF SIMPLI - PARA PC LOCAL');
    console.log('========================================');
    ...
    console.log('========================================');
    console.log('⚡ Chromium path: /snap/bin/chromium');
    console.log('📁 Archivos en: /home/anthony/botdata/');
    console.log('💾 Perfil Chromium: /home/anthony/chromium-profile');
    console.log('========================================');
    ...
    console.log('✨ Revisa /home/anthony/botdata/bot_log.txt');
    console.log('✨ Tarjetas válidas en /home/anthony/botdata/lives.txt');
    console.log('✨ Cuentas en /home/anthony/botdata/cuentas_pdfsimpli.json');
    console.log('========================================');
})();