...
class BotPDFSimpli {
    ...
    // SISTEMA DE reCAPTCHA V2 CHECKBOX - COMPLETAMENTE REESCRITO
    async manejarRecaptcha() {
        try {
            ...
            const checkboxByClass = await recaptchaFrame.$('.recaptcha-checkbox');
            if (checkboxByClass) {
                this.log('✅ Checkbox encontrado por clase', 'SUCCESS');
                ...
                this.log('✅ Clic en checkbox (clase) realizado', 'SUCCESS');
                await this.delay(5000);
                
                const resuelto = await recaptchaFrame.evaluate(() => {
                    const checkbox = document.querySelector('.recaptcha-checkbox');
                    return checkbox ? (checkbox.getAttribute('aria-checked') === 'true' || checkbox.classList.contains('recaptcha-checkbox-checked')) : false;
                });
                
                if (resuelto) {
                    this.log('✅ reCAPTCHA resuelto automáticamente (clase)', 'SUCCESS');
                    this.estadisticas.recaptchasResueltos++;
                    return true;
                }
            }

            // Estrategia 3: Fallback - espera manual más corta
            this.log('⏳ reCAPTCHA no se pudo resolver automáticamente - esperando resolución manual (15 segundos)...');
    ...
    async ejecutarProcesoCompleto() {
        try {
            ...
            this.log('🤖 BOT PDF SIMPLI - VERSIÓN CORREGIDA');
...
(async () => {
    console.log('========================================');
    console.log('🤖 BOT PDF SIMPLI - VERSIÓN CORREGIDA');
    console.log('========================================');
