...
class BotPDFSimpli {
    ...
    // SISTEMA DE REGISTRO - VERSIÓN MEJORADA (como TTTT.py)
    async manejarRegistro() {
        return await this.ejecutarConReintentos(async () => {
            try {
                ...
                // BUSCAR CAMPOS MÁS AGRESIVAMENTE
                ...
                // MANEJAR reCAPTCHA CON EL NUEVO MÉTODO MEJORADO
    ...
    // SISTEMA DE reCAPTCHA V2 CHECKBOX - VERSIÓN MEJORADA CON COORDENADAS ABSOLUTAS
    async manejarRecaptcha() {
        try {
            ...
            this.log(`📊 Analizando ${frames.length} iframes...`);

            for (const frame of frames) {
                try {
                    const frameUrl = frame.url();
                    if (frameUrl.includes('recaptcha/api2/anchor')) {
                        anchorFrame = frame;
                        this.log('✅ Iframe reCAPTCHA anchor encontrado', 'SUCCESS');
                        break;
                    }
                } catch (error) {
                    // Continuar con el siguiente frame si hay error
                    continue;
                }
            }

            if (!anchorFrame) {
                this.log('✔ No hay reCAPTCHA visible. Continuando...', 'SUCCESS');
                return true;
            }

            this.log('🎯 Preparando click humano con boundingBox REAL...');

            // Esperar el selector del checkbox
            ...
            const yaMarcado = await anchorFrame.$eval('#recaptcha-anchor', el =>
            ...
            try {
                // Obtener bounding box del iframe usando APIs internas de Puppeteer
                const frameElement = await anchorFrame._client.send(
                    'DOM.getFrameOwner',
                    { frameId: anchorFrame._id }
                );

                const nodeInfo = await this.page._client.send('DOM.getBoxModel', {
                    nodeId: frameElement.nodeId
                });

                const frameBox = nodeInfo.model.content; // [x1,y1,x2,y2,x3,y3,x4,y4]
                const frameX = frameBox[0];
                const frameY = frameBox[1];

                // Obtener bounding box del checkbox dentro del iframe
                const checkbox = await anchorFrame.$('#recaptcha-anchor');
                const box = await checkbox.boundingBox();

                if (!box) {
                    this.log('❌ No se obtuvo boundingBox del checkbox', 'ERROR');
                    return true;
                }

                // Calcular coordenadas absolutas reales
                const clickX = frameX + box.x + box.width / 2;
                const clickY = frameY + box.y + box.height / 2;

                this.log(`📍 Click final en (${Math.round(clickX)}, ${Math.round(clickY)})`);

                // Micro movimiento previo (fix Termux)
                await this.page.mouse.move(clickX - 10, clickY - 10);
                await this.delay(50);

                this.log("🖱️ Click humano...");

                // Movimiento principal y click
                ...
                const marcadoAhora = await anchorFrame.$eval('#recaptcha-anchor', el =>
                    el.getAttribute('aria-checked') === 'true'
                );

                if (marcadoAhora) {
                    this.log('✅ reCAPTCHA resuelto correctamente', 'SUCCESS');
                    this.estadisticas.recaptchasResueltos++;
                    return true;
                }

                // Detectar si se abrió challenge
                const challengeFrame = this.page.frames().find(f =>
                    f.url().includes('recaptcha/api2/bframe')
                );

                if (challengeFrame) {
                    this.log('⚠️ Se abrió challenge - esperando 5 segundos y continuando', 'WARNING');
                    await this.delay(5000);
                    this.estadisticas.recaptchasManuales++;
                    return true;
                }

                this.log('❌ No se pudo marcar, pero continúo...', 'ERROR');
                return true;

            } catch (error) {
                this.logError(error, 'click preciso en reCAPTCHA');
                this.log('🔄 Intentando método alternativo...', 'WARNING');
                
                // Fallback: intentar click directo
                try {
                    await anchorFrame.click('#recaptcha-anchor');
                    await this.delay(2000);
                    this.log('✅ reCAPTCHA clickeado con método alternativo', 'SUCCESS');
                    return true;
                } catch (fallbackError) {
                    this.logError(fallbackError, 'método alternativo reCAPTCHA');
            ...
            return true; // Siempre continuar incluso con errores
        }
    }
    ...
    async ejecutarProcesoCompleto() {
        try {
            ...
            this.log('🤖 BOT PDF SIMPLI - VERSIÓN MEJORADA CON LÓGICA DE TTTT.py');
            this.log('✅ Registro automático mejorado');
            this.log('🎯 Sistema de reCAPTCHA corregido');
            this.log('🔄 Detección agresiva de iframes');
...
(async () => {
    ...
    console.log('🔄 Detección agresiva de iframes');
