...
class ADBManager {
    ...
    async checkAndChangeIP() {
        ...
        if (this.tarjetaCounter % 6 === 0) {
            await this.log(`${COLORS.yellow}📊 Contador de tarjetas: ${this.tarjetaCounter} - SOLICITANDO CAMBIO DE IP${COLORS.reset}`);
...
class PDFSimpliBot {
    ...
    async initialize() {
        ...
        this.browser = await puppeteer.launch({
            ...
            headless: false, // ✅ MODO VISIBLE PARA VNC
            args: [
                "--no-sandbox",
                ...
                "--start-maximized", // ✅ NUEVO - Maximiza la ventana
                "--remote-debugging-port=9222" // ✅ NUEVO - Para debugging si es necesario
            ]
        });

        this.page = await this.browser.newPage();
        await this.page.setDefaultNavigationTimeout(0); // ✅ Sin timeouts de navegación
        ...
        await this.log("🚀 KATY20 — Iniciado en modo VISIBLE con sistema ADB integrado");
    }
    ...
    async verificarResultadoPago(tarjetaActual) {
        ...
        await this.log("⏳ Esperando 15 segundos para que procese el pago...");
    ...
    async executeFlowCuenta() {
        // ✅ VERIFICAR CAMBIO DE IP CADA 6 TARJETAS
